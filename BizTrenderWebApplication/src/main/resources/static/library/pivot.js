...
(function() {
  ...
  */
  ...
  aggregatorTemplates = {
      ...
      return function(_arg) {
        ...
        return function() {
          return {
            ...
            value: function() {
              return Math.ceil(this.sum / 10) * 10;
            },
    ...
    limitSum: function(sigfig, scaler) {
    	sigfig = 0;
        /*if (sigfig == null) {
          sigfig = 3;
        }*/
        ...
        return function(_arg) {
        	console.log("CALL AGGREGATOR LIMITSUM -> ", rowsForAggregator);
          var attr;
          attr = 1 <= _arg.length ? __slice.call(_arg, 0) : [];
          return function() {
            return {
              sum: 0,
              push: function(record, idx) {
            	  console.log("attr[idx]", attr[idx]);
            	  console.log("record", record);
            	  var sizeNonEmptyVals = 0, sizeEmptyVals = 0, sizeRecord = 0;
            	  if(attr[idx].toLowerCase().includes('limit')) {
            		  for(var k in record){
                		  if((rowsForAggregator.includes(k) || k == attr[idx]) && record[k] != "") {
                			  sizeNonEmptyVals++;
                		  }
                		  else if (record[k] == "") {
                			  sizeEmptyVals++;
            	  ...
            	  else if (attr[idx].toLowerCase().includes('var')) {
            		  for(var k in record){
                		  if((rowsForAggregator.includes(k) || k == attr[idx] || k == "Date") && record[k] != "") {
                			  sizeNonEmptyVals++;
                		  }
                		  else if (record[k] == "") {
                			  sizeEmptyVals++;
            	  ...
            	  var sizeVals = sizeNonEmptyVals + sizeEmptyVals + 1;
            	  console.log("sizeNonEmptyVals", sizeNonEmptyVals);
            	  console.log("sizeEmptyVals", sizeEmptyVals);
            	  console.log("sizeVals", sizeVals);
            	  console.log("sizeRecord", sizeRecord);
                if (!isNaN(parseFloat(record[attr[idx]])) && sizeVals == sizeRecord) {
                  return this.sum += parseFloat(record[attr[idx]]);
                }
              },
              value: function() {
  ...
  aggregators = {
    ...
    limitSum: aggregatorTemplates.limitSum(3),
    count: function() {
      return function() {
        return {
          count: 0,
...
/*    countUnique: function(_arg) {
  ...
  renderers = {
    ...
    "Column Chart": function(pvtData, opts) {
    		$.pivotUtilities.highchart_renderers['Column Chart']
    },
    "Line Chart": function(pvtData, opts) {
  		$.pivotUtilities.highchart_renderers['Line Chart']
    }
  };
  ...
  */
  ...
  getPivotData = function(input, cols, rows, vals, aggregator, aggregatorKeys, filter, derivedAttributes) {
	  rowsForAggregator = $.map(rows, function(value, index) {
		    return [value];
	  });
    var pivotData;
  ...
  pivotTableRenderer = function(pivotData, opts) {
    ...
    for (j in colAttrs) {
      ...
      if (parseInt(j) === 0) {
        tr.append($("<th class='pvtTotalLabel colTotalLabel'>").text(opts.localeStrings.totals).attr("colspan", col_colspan).attr("rowspan", col_rowspan));
      }
      result.append(tr);
    }
    ...
    for (i in rowKeys) {
      ...
      for (j in rowKey) {
        ...
        if (x !== -1) {
          th = $("<th class='pvtRowLabel' style='cursor:pointer'>").text(txt).attr("rowspan", x).attr("onclick","callbackFun("+"'"+txt+"'"+")");
    ...
    tr = $("<tr>");
    th = $("<th class='pvtTotalLabel rowTotalLabel'>").text(opts.localeStrings.totals);
  ...
  */
  ...
  $.fn.pivotUI = function(input, inputOpts, overwrite) {
    ...
    try {
      ...
      _fn = function(c) {
        ...
        if (keys.length > opts.menuLimit) {
          ...
          for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
            ...
            valueList.append($("<p style='text-align: left;'>").append(filterItem));
          }
        }
        ...
        // drill down for attributes
        return attrElem.bind("click", function(e) {
        	e.stopPropagation();
      ...
      selectCompareControl = $("<td class='compareBox'>");
      ...
      tr2 = $("<tr>");
      tr2.append($("<td  class='pvtAxisContainer pvtHorizList pvtRows'>").css("text-align", "center"));
      tr2.append($("<td  class='pvtAxisContainer pvtHorizList pvtCols'>"));
      uiTable.append(tr2);
      tr3 = $("<tr>");
      tr3.append($("<td style='width:30px; vertical-align:top;' class='pvtAxisContainer pvtVals'>"));
      
      pivotTable = $("<td style='width: 30px;' valign='top' id='pvtRenderArea' class='pvtRendererArea'>");
      ...
      refresh = function() {
        var exclusions, natSort, subopts, unusedAttrsContainer, vals, pivotUIOptions;
        subopts = {
      ...
      var pvtAggregatorOnLoad = function(opts, callback){
          var cuentaSelect = 0, select;
          $('td.pvtAxisContainer.pvtHorizList.pvtVals.ui-sortable').find("select").each(function() {
            return ++cuentaSelect;
          });
          select = $("<select class='pvtAggregator" + (++cuentaSelect) + "'>").bind("change", function() {
            var aggregatorSelected, newItemConSelect, newSelect;
          ...
          $(".pvtAxisContainer.pvtVals li.label-info > div").append(select);
          // default pivot loading case
          $("<br/><br/><br/><br/>").insertAfter(".pvtAxisContainer.pvtVals > li")
          return refresh();
      }

      pvtAggregatorOnLoad(opts, function(selectPreVals){
        if(selectPreVals){
          refresh();
        }
      });
