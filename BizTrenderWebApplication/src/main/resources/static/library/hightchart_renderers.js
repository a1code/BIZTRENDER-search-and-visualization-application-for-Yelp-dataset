(function ($) {
    ...
    makeHighChart = function (chartType, extraOptions) {
        return function (pivotData, opts) {
            ...
            //titles
            var temp = "";
            //for(var i=0; i<pivotData.aggregatorKeys.length; i++) {
            	temp += pivotData.aggregatorKeys[0] + "(" + pivotData.valAttrs[0] + ")" + ", ";
             //}
            title = vAxisTitle = temp.slice(0, -2);
            
            //title = titleFormatter(pivotData.aggregatorName);

            hAxisTitle = pivotData.colAttrs.map(titleFormatter).join(",");
            if (hAxisTitle !== "") {
                title += " " + opts.localeStrings.by + " " + pivotData.rowAttrs.map(titleFormatter).join(",");
            ...
            var options = {
                ...
                xAxis: {
                    ...
                    labels: {
						formatter: function () {
							return '<a style="cursor: pointer" onclick="pivotChartLabelClick('+"'"+this.value+"'"+')">' + this.value + '</a>';
						},
						useHTML: true
					}
                },
                    ...
                    labels: {
						...
						formatter: function() {
		                    ...
		                    if(this.value >=1000) {
		                        ...
		                        while (i-- && ret === undefined) {
		                            var multi = -1 * Math.pow(1000, i + 1);
		                            if (this.value <= multi && numericSymbols[i] !== null) {
		                                ret = -1 * (this.value / multi) + numericSymbols[i];
		                            }
		                        }
		                    }
